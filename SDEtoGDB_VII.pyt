...
class SDETOGDB(object):
    ...
    def getParameterInfo(self):
        ...
        param4 = arcpy.Parameter(
            displayName="Number of Days to Sync",
            name="Numdays",
            datatype="GPLong",
            parameterType="Required",
            direction="Input")
    ...
    def execute(self, parameters, messages):
        ...
        Numdays=int(parameters[4].valueAsText)
        inFolder=parameters[5].valueAsText
        outFolder=parameters[6].valueAsText
        ...
        def dbLastUpdateDate(db,Numdays):
            maxdays=1000
            now = datetime.datetime.now()
            arcpy.AddWarning("Checking if there are any edits in {} in the past {} days".format(str(db),str(Numdays)))
            arcpy.AddMessage("...")
            for version in arcpy.da.ListVersions(db):
                
                days_sinceLastEdit=(now-version.lastModified).days
            
                
                if days_sinceLastEdit<= maxdays:
                    maxdays=days_sinceLastEdit
            
            if maxdays<=Numdays:
                arcpy.AddMessage("..."+ str(db)+"...")
                arcpy.AddMessage("Days Since Last Edit ===>  "+ str(maxdays))
                return True
            else:
                arcpy.AddMessage("..."+ str(db)+"...")
                arcpy.AddMessage("No Recent Edits ===>  "+ str(maxdays)+" Days is Grater than "+str(Numdays))
                return False
        ...
        def dbCopy(inFolder,outFolder,Numdays):
            ...
            for db in Workspace: 
                datasync=dbLastUpdateDate(db,Numdays)
                
                if datasync==True:
                    sdeWorkspace.append(db)

            if len(sdeWorkspace)>0:
                arcpy.AddMessage("...")
                arcpy.AddMessage("The Following Databases will be synced...")
                arcpy.AddMessage(sdeWorkspace)
                
                arcpy.AddMessage("...")

                arcpy.AddWarning("{} Databases out of {} Will be Synced".format(len(sdeWorkspace),len(Workspace)))
                arcpy.AddMessage("...")

                #Collect all sde file names in a list then delete all gdb that maches the list
                gdb_delete=[]
                for sde in sdeWorkspace:
                ...
                arcpy.AddWarning("No Databases will be synced because no recent edits...")



        StopStartServer(server, port, adminUser, adminPass,operation="stop")
        arcpy.AddMessage(Numdays)
        dbCopy(inFolder,outFolder,Numdays)
